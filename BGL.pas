...
procedure SquareFill(A, B, C: tPoint);
procedure BarycentricFill(A, B, C: tPoint);
procedure FastFill(A, B, C: tPoint);
...
var
   ...
   GammaCurve: array [0..MAX_RGB] of integer;
...
var
   i: integer;
begin
   for i := 0 to MAX_RGB do
      GammaCurve[i] := round(power(i/MAX_RGB, 1/gamma)*MAX_RGB);
end;
...
//Процедуры закраски многоугольников
procedure SquareFill(A, B, C: tPoint);
var
   maxPoint, minPoint: tPoint;
   i, j: integer;
   aSide, bSide, cSide: integer;
begin
   maxPoint.x := Max(A.x, B.x, C.x);
   maxPoint.y := Max(A.y, B.y, C.y);
   minPoint.x := Min(A.x, B.x, C.x);
   minPoint.y := Min(A.y, B.y, C.y);
   for i := minPoint.x to maxPoint.x do
      for j := minPoint.y to maxPoint.y do begin
         aSide := (A.y - B.y)*i + (B.x - A.x)*j + (A.x*B.y - B.x*A.y);
         bSide := (B.y - C.y)*i + (C.x - B.x)*j + (B.x*C.y - C.x*B.y);
         cSide := (C.y - A.y)*i + (A.x - C.x)*j + (C.x*A.y - A.x*C.y);
         if (aSide >= 0) and (bSide >= 0) and (cSide >= 0) or (aSide < 0) and (bSide < 0) and (cSide < 0) then
            PutPixel(i, j, CC);
      end;
end;
...
procedure BarycentricFill(A, B, C: tPoint);
var
   x, y, x_min, x_max, y_min, y_max: integer;
   denom, u, v, w: real;
begin
   x_min := Max(0, Min(Min(A.x, B.x), C.x));
   x_max := Min(Display.SizeX - 1, Max(Max(A.x, B.x), C.x));
   y_min := Max(0, Min(Min(A.y, B.y), C.y));
   y_max := Min(Display.SizeY - 1, Max(Max(A.y, B.y), C.y));

   for y := y_min to y_max do begin
      for x := x_min to x_max do begin
...
procedure FillLine(y, x1, x2: integer);
begin
...
begin
   ...
   if B.y <> A.y then
      for y := A.y to B.y do begin
         x1 := A.x + (B.x - A.x) * (y - A.y) div (B.y - A.y);
         x2 := A.x + (C.x - A.x) * (y - A.y) div (C.y - A.y);
         FillLine(y, x1, x2);
      end;
   if C.y <> B.y then
    for y := B.y to C.y do begin
      x1 := B.x + (C.x - B.x) * (y - B.y) div (C.y - B.y);
      x2 := A.x + (C.x - A.x) * (y - A.y) div (C.y - A.y);
      FillLine(y, x1, x2);
    end;
end;
